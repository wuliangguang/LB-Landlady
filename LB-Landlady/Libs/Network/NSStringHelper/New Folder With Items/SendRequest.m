        ...
        if (_IS_CSRFTOKEN )
        {
            ...
            if (errorCode == 403)
            {
                ...
                if (cookieUrl)
                {
                    if (_requestNumb == 0)
                    {
                        [self LB_PostWithURLString:url WithParams:params WithSuccess:^(id responseObject) {
                            NSString *html = operation.responseString;
                            NSData* data=[html dataUsingEncoding:NSUTF8StringEncoding];
                            id dict=[NSJSONSerialization  JSONObjectWithData:data options:0 error:nil];
                            success(dict);
...
- (void)feedBackErrorStatusCN:(NSDictionary *)responseObject WithController:(UIViewController *)controller
{
    ...
    hub.labelFont =[UIFont systemFontOfSize:13];
    hub.detailsLabelFont = [UIFont systemFontOfSize:13];
...
- (void)showErrorInfoWith:(NSInteger)statusCode VC:(UIViewController *)controller
{
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        {
            hub.labelText = @"网络请求超时，请稍候重试！";
        }
        else if (statusCode == -1002)
        {
            hub.labelText = @"不支持的URL！";
        }
        else if (statusCode == -1003)
        {
            hub.labelText = @"未能找到指定的服务器主！";
        }
        else if (statusCode == -1004)
        {
            hub.labelText = @"服务器连接失败！";
        }
        else if (statusCode == -1005)
        {
            hub.labelText = @"连接丢失，请稍候重试！";
        }
        else if (statusCode == -1009)
        {
            hub.labelText = @"互联网连接似乎是离线!";
        }
        else if (statusCode == -1012)
        {
            hub.labelText = @"操作无法完成！";//allowsInvalidSSLCertificate = yes
        }
        else
        {
            hub.labelText = @"网络请求发生未知错误，请稍候再试下吧！";
        }
        [hub hide:YES afterDelay:3];
    });
}
